...
;(function ($, _) {
	...
	var PageViewModel = function () {
		...
		var _this = this;
		_this.pages = ['Home', 'Gallery', 'Testimonials', 'Partners', 'Contact Us'];
		_this.chosenPageId = ko.observable();
		_this.chosenPageTitle = ko.observable();
		_this.chosenPageContent = ko.observable();
		_this.pageChanged = ko.observable(!1);
		_this.lastPage = ko.observable();

		_this.changePage = function (page, e) {
			if (page !== _this.lastPage) {
				$('#content').fadeOut(1E3, function () {
					location.hash = page;
				});
			}
		};

		var Property = function(name, tag) {
			this.name = name;
			this.tag = tag;
		};

		_this.availableProperties = ko.observableArray([
			new Property("Standard", 'one'),
			new Property("Awesome", 'two'),
			new Property("Major", 'three')
		]);
		...
		_(function () {
			...
			this.get('#:page', function () {
				...
				if (_this.lastPage !== page) {
					_this.chosenPageId(page);
					_this.chosenPageTitle(page);
					$.get('/html/' + page + '.html', function (data) {
						_this.chosenPageContent(data);
						_this.lastPage(page);
						_this.pageChanged(!0);
					});
				} else {
	...
	ko.applyBindings($.view = new PageViewModel());
